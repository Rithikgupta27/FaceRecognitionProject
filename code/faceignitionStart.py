...
while True:
    ...
    try:
        ...
        if confidence>=83:
            cv2.putText(image,"Welcome sir",(250,450),cv2.FONT_HERSHEY_SCRIPT_COMPLEX,1,(0,255,255))
            cv2.imshow('face',image)
            x+=1
        else:
            cv2.putText(image,"Owner not Match",(250,450),cv2.FONT_HERSHEY_SCRIPT_COMPLEX,1,(0,255,255))
            cv2.imshow('face',image)
            c+=1
    except:
...
if x>=5:
    ...
    var = 'a'
    c=var.encode()
    speak("Face recognition complete..it is matching with database...welcome..sir..engine is starting")
    speak("Face recognition complete..it is matching with database...welcome..sir..engine is starting")
    ard.write(c)
    time.sleep(5000)
elif c==30:
